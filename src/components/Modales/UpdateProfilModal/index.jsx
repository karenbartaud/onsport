...
function UpdateProfilModal({ toggleUpdateProfilModal, isShowUpdateProfilModal }) {
  ...
  React.useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get('https://ronaldfk-server.eddi.cloud:8443/api/location/');
  ...
  const getCitiesFromSearch = async () => {
    ...
    try {
      const response = await axios.get(`https://ronaldfk-server.eddi.cloud:8443/api/location?search=${citySearch}`);
      setListLocation(response.data);
    } catch (error) {
      console.error(error);
    }
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      const response = await axios({
        method: 'PATCH',
        url: `https://ronaldfk-server.eddi.cloud:8443/api/user/profil/${userId}`,
        headers: {
